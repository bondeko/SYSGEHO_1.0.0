...
import com.bondeko.sysgeho.ui.imp.vue.FacVisMedEmbVue;


public class FacVisMedEmbCtrl extends SysGehoCtrl<TabFacVisMedEmb, TabFacVisMedEmb> {


	/**
	 * Nom du Bean managé par JSF dans le fichier de Configuration 
	 */
	private static String nomManagedBean = "facVisMedEmbCtrl";	

	public FacVisMedEmbCtrl() {
		defaultVue = new FacVisMedEmbVue();					
	}
	...
	public IBaseSvco<TabFacVisMedEmb> getEntitySvco() throws ServiceLocatorException{	
		return FactureSvcoDeleguate.getSvcoFacVisMedEmb();
	}
	
	public Class<FacVisMedEmbCtrl> getMyClass() {
		return FacVisMedEmbCtrl.class;
	}
	...
	public List<Traitement> getListeTraitements() {
		String v$codeEntite = "FacVisMedEmb";

		System.out.println("FacVisMedEmbCtrl.getListeTraitements() ici il vaut : " + v$codeEntite);
		// Ensemble des traitements standards
		Map<String, Traitement> v$mapTrt = new TreeMap<String, Traitement>(
				FactureTrt.getTrtStandards(v$codeEntite));
		
		v$mapTrt.put(FactureTrt.VALIDER_FACTURE_VM_EMB.getKey(), new Traitement(FactureTrt.VALIDER_FACTURE_VM_EMB));
		
		v$mapTrt.put(FactureTrt.PAYER_FACTURE_VM_EMB.getKey(), new Traitement(FactureTrt.PAYER_FACTURE_VM_EMB));
	...
	public void buildListeTraitement() {
		if(getMapTraitements() == null){
			setMapTraitements(FactureTrt.getTrtStandards("FacVisMedEmb")) ;
		}
	}
	
	@Override
	public List<TabFacVisMedEmb> rechercherParCritere(TabFacVisMedEmb p$entity)
	...
	public String valider() {
		...
		try {
			...
			TabFacVisMedEmb entity = defaultVue.getEntiteCourante();
			...
			defaultVue.setEntiteCourante(FactureSvcoDeleguate.getSvcoFacVisMedEmb()
					.valider(defaultVue.getEntiteCourante()));

			v$msgDetails += defaultVue.getEntiteCourante().getRefFacVisMedEmb()+" a été validé";
	...
	public String payer() {
		...
		try {
			...
			v$msgDetails += defaultVue.getEntiteCourante().getRefFacVisMedEmb()+" a été payé";
	...
	public void coherenceIHM() {
		...
		FacVisMedEmbVue v$vue = (FacVisMedEmbVue) defaultVue;
		
				
		/*
		 * MISE A JOUR DE LA LISTE DES PIECES JOINTES
		 */
		TabFacVisMedEmb v$facture = v$vue.getEntiteCourante();		
		try{	
			// Recherche Par ID pour récupérer le lot Documentaire à jour contenant les Pièces jointes
			v$facture = FactureSvcoDeleguate.getSvcoFacVisMedEmb().rechercher(FacVisMedEmbVue.getTabFacVisMedEmbForSearch(),v$vue.getEntiteCourante().getId());			
	...
	public String ajouterPceJte(){
		...
		try{
			...
			defaultVue.setEntiteCourante(FactureSvcoDeleguate.getSvcoFacVisMedEmb().ajouterPieceJte(defaultVue.getEntiteCourante()));	
