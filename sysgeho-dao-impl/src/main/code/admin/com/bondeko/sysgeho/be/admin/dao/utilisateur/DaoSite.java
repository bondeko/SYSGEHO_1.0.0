...
@Stateless
public class DaoSite extends BaseDao<TabSite, String> implements IDaoSite{
	
	private static BaseLogger logger = BaseLogger.getLogger(DaoSite.class); 
	...
	public <X extends BaseEntity> List<X> findByExample(X entity)
		...
		TabSite currentSite = (TabSite)entity;
		if(currentSite.getCodSite()!= null && !currentSite.getCodSite().equals("")){
			clauseWhere = clauseWhere + "AND upper(o.codSite) like '%"+currentSite.getCodSite()+"%'";
		}
		if(currentSite.getLibSite()!= null && !currentSite.getLibSite().equals("")){
			clauseWhere = clauseWhere + "AND upper(o.libSite) like '%"+currentSite.getLibSite()+"%'";
		}
		try{
