...
@Stateless
public class DaoRol extends BaseDao<TabRol, String> implements IDaoRol{
	
	private static BaseLogger logger = BaseLogger.getLogger(DaoRol.class); 
	...
	public <X extends BaseEntity> List<X> findByExample(X entity)
		...
		TabRol currentRol = (TabRol)entity;
		if(currentRol.getCodRol()!= null && !currentRol.getCodRol().equals("")){
			clauseWhere = clauseWhere + "AND upper(o.codRol) like '%"+currentRol.getCodRol()+"%'";
		}
		if(currentRol.getLibRol()!= null && !currentRol.getLibRol().equals("")){
			clauseWhere = clauseWhere + "AND upper(o.libRol) like '%"+currentRol.getLibRol()+"%'";
		}
		try{
