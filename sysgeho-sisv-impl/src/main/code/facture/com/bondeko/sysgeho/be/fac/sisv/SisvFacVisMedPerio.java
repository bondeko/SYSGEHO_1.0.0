...
import com.bondeko.sysgeho.be.fac.dao.IDaoFacVisMedPerio;
...
@Stateless
public class SisvFacVisMedPerio extends BaseSisv<TabFacVisMedPerio, String> implements ISisvFacVisMedPerio{
	
	private static BaseLogger logger = BaseLogger.getLogger(SisvFacVisMedPerio.class);
	...
	@EJB
	IDaoFacVisMedPerio daoFacVisMedPerio; 
	...
	@Override
	public IBaseDao<TabFacVisMedPerio, String> getBaseDao() {
		return daoFacVisMedPerio;
	}
	...
	public <X extends BaseEntity> X creer(X p$entite) throws BaseException  {
		...
		initialiserDonnees((TabFacVisMedPerio)p$entite);
	...
	public <X extends BaseEntity> X rechercher(X entity, Serializable id) throws SysGehoSystemException {
		try {
			return daoFacVisMedPerio.findById(entity, id);
	...
	public <X extends BaseEntity> List<X> rechercherTout(X entity) throws SysGehoSystemException {
		...
		try {
			return daoFacVisMedPerio.findAll(entity);
		} catch (SysGehoPersistenceException e) {
			e.printStackTrace();
			SysGehoSystemException sbr = new SysGehoSystemException(e);
			throw sbr;
	...
	public <X extends BaseEntity> List<X> rechercherParCritere(X entity)
		...
		try {
			return daoFacVisMedPerio.findByExample(entity);
		} catch (SysGehoPersistenceException e) {
			e.printStackTrace();
			SysGehoSystemException sbr = new SysGehoSystemException(e);
			throw sbr;
	...
	private TabFacVisMedPerio initialiserDonnees(TabFacVisMedPerio visMed){
	...
	@Override
	public TabFacVisMedPerio valider(TabFacVisMedPerio $pFacVisMedPerio) throws SysGehoSystemException  {
		try {
			$pFacVisMedPerio.setBooVal(BigDecimal.ONE);
			$pFacVisMedPerio.setEtatEnt(EnuEtat.VALIDE.getValue());
			return getBaseDao().update($pFacVisMedPerio);
		} catch (SysGehoPersistenceException e) {
	...
	@Override
	public TabFacVisMedPerio payer(TabFacVisMedPerio $pFacVisMedPerio) throws SysGehoSystemException  {
		try {
			$pFacVisMedPerio.setBooPaie(BigDecimal.ONE);
			$pFacVisMedPerio.setEtatEnt(EnuEtat.PAYE.getValue());
			return getBaseDao().update($pFacVisMedPerio);
		} catch (SysGehoPersistenceException e) {
	...
	public TabFacVisMedPerio ajouterPieceJte(TabFacVisMedPerio p$entite) throws BaseException {
		try {
			...
			return daoFacVisMedPerio.update(p$entite);
	...
	@Override
	public void saveLotDoc(TabFacVisMedPerio facVM) {
		try {
			...
			if (facVM.getListNewPce().size() > 0) {
				...
				if (facVM.getTabLotDoc() == null
					...
					lot.setLLotDoc("Lot documentaire de la facture " + facVM.getRefFacVisMedPerio());
